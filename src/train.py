...
if __name__ == '__main__':
    ...
    model = xception(pretrained=True).to(device)
    ...
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.5, verbose=True)
    train_writer = Tensorboard('./logs/logs_train')
    val_writer = Tensorboard('./logs/logs_val')
    acc_writer = Tensorboard('./logs/logs_acc')

    model_name = f'./trained_models/xception-{time.time()}'
